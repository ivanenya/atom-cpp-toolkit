...
export default {
  ...
  switchheader() {
      ...
      var newExtName = '';
      var isHeader = false;
      if ( extName === 'h' ) { newExtName = 'cpp'; isHeader = true; }
      else if ( extName === 'hpp' ) {newExtName = 'cc'; isHeader = true; }
      ...
      if ( f.existsSync() == false )
      {
          ...
          if ( isHeader && dir === "include" )
          {
              dirs[dirs.length-2] = "src";
          }
          else if ( false == isHeader && dir == "src" )
          {
              dirs[dirs.length-2] = "include";
          }
          newFilePath = dirs.join( "/" );
  ...
  sformatex( editor )
  {
      ...
      for ( var i = 0; i < rows.length-1; i ++ )
      {
          ...
          if ( equalPos != -1 )
          {
              if ( equalPos > 1 && rowNoComment[equalPos-1] == "!" || equalPos + 1 < rowNoComment.length && rowNoComment[equalPos+1] == "=" )
              {
                  equalPos = -1;
              }
              else if ( equalPos > 1 )
              {
                  if ( "+-*/%^|&".indexOf(rowNoComment[equalPos-1]) != -1 )
                  {
                      equalPos = equalPos - 1;
                      equalLen = 2;
                  }
                  ...
                  {
                      if ( rowNoComment[equalPos-1] == "<" && rowNoComment[equalPos-2] == "<"
                        || rowNoComment[equalPos-1] == ">" && rowNoComment[equalPos-2] == ">" )
                        {
                            equalPos = equalPos - 2;
                            equalLen = 3;
                        }
          ...
          if ( equalPos == -1 )
          {
              ...
              l.push( rowNoComment.substr(equalPos, equalLen) );
              var newPos = equalPos + equalLen;
              while ( newPos < rowNoComment.length && rowNoComment[newPos] == " " )
              {
                  newPos = newPos + 1;
              }
              if ( newPos < rowNoComment.length ) {
                  l = l.concat(rowNoComment.substring( newPos ));
              }
          }
          if ( comment != "" )
          {
          ...
          equalIndex.push( index );
          if ( index != -1 && maxEqualIndex < index )
          {
              maxEqualIndex = index;
          }
          */
      ...
      {
          if ( equalIndex[i] != -1 && equalIndex > maxEqualIndex )
          {
